...
const MarkedCards = (props) =>{
    const {dispatch} = useHabitContext()

    const handleMarkedDelete = async () => {
        const response = await fetch('/habit/marked/'+props._id,{
            method:'DELETE'
        })
        const json = await response.json()
        if(response.ok)
        {
            dispatch({type: 'DELETE_MARKEDHABIT',payload: json})
        }
    }
                ...
                <div className="title">
                    <div className="alter_head">
                        ...
                        <div className="alter_head_but">
                            ...
                            <button onClick={handleMarkedDelete}><img src={del}/></button>
                        </div>
                    </div>
                        <h2>{props.habit}</h2>
                </div>
